...
public class CalculatorAsyncResourceImpl implements ICalculatorAsyncResource {
	...
	public CalculatorAsyncResourceImpl(final Client client, final String uri) {
		...
		this.sum = new AsynchronousEvaluator2<>(new IAsynchronousFunction2<Double, Double, Double>() {
			@Override
			public void y(Double x, Double y, ICallback<Double> callback) {
				...
				invoker.method("GET", new ICallbackAdapter<Double>(callback) {}); 			
			}		
		});
				  
		this.difference = new AsynchronousEvaluator2<>(new IAsynchronousFunction2<Double, Double, Double>() {
			@Override
			public void y(Double x, Double y, ICallback<Double> callback) {
				WebTarget target = client.target(uri);
		...
		this.product = new AsynchronousEvaluator2<>(new IAsynchronousFunction2<Double, Double, Double>() {
			@Override
			public void y(Double x, Double y, ICallback<Double> callback) {
				WebTarget target = client.target(uri);
		...
		this.quotient = new AsynchronousEvaluator2<>(new IAsynchronousFunction2<Double, Double, Double>() {
			@Override
			public void y(Double x, Double y, ICallback<Double> callback) {
				WebTarget target = client.target(uri);
	...
	@Override
	public IEvaluation<Double> difference(
			final Double x,
			final Double y
	) {
		return difference(
	...
	@Override
	public IEvaluation<Double> product(
			final Double x,
			final Double y
	) {
		return product(
	...
	@Override
	public IEvaluation<Double> quotient(
			final Double x,
			final Double y
	) {
		return quotient(
