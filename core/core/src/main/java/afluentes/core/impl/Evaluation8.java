...
abstract class Evaluation8<X1, X2, X3, X4, X5, X6, X7, X8, Y> extends Evaluation<Y> {
    ...
    protected boolean isEvaluable() {    
        return true && x1.status == EVALUATED && x2.status == EVALUATED && x3.status == EVALUATED && x4.status == EVALUATED && x5.status == EVALUATED && x6.status == EVALUATED && x7.status == EVALUATED && x8.status == EVALUATED;        		
    }
